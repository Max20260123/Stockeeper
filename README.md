...
<head>
    ...
    <title>月供股票智能系統 v1.0 (精確財務版)</title>
    ...
    <style>
        ...
        /* 排序圖示 */
        .sort-icon::after { content: ' ↕'; font-size: 0.8em; color: #ccc; }
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="dashboard-grid">
            <div class="kpi-box">
                <div class="kpi-title">總投入成本 (本金)</div>
                <div class="small-text" style="margin-bottom:5px">(含買入及賣出手續費)</div>
                <div class="kpi-val" id="kpi-cost">$0.00</div>
            </div>
            <div class="kpi-box">
                <div class="kpi-title">當前持倉市值</div>
                <div class="kpi-val" id="kpi-market">$0.00</div>
            </div>
            <div class="kpi-box">
                <div class="kpi-title">總已收股息</div>
                <div class="kpi-val text-up" id="kpi-div">$0.00</div>
            </div>
            <div class="kpi-box">
                <div class="kpi-title">淨回報 (含已實現損益)</div>
    ...
    <div class="card" style="border-left: 5px solid var(--primary);">
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 15px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 150px;">
                <label class="small-text">日期</label>
                <input type="date" id="batch-date">
            </div>
            <div style="flex: 1; min-width: 150px;">
                <label class="small-text">該次總手續費 ($)</label>
        ...
        <table id="batch-input-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%">代號</th>
                    <th style="width: 25%">金額 / 單價</th>
        ...
        <div class="btn-row">
            <button class="btn btn-outline" onclick="addBatchRow()">+ 加入一行</button>
            <button class="btn btn-primary" onclick="saveBatchTransaction()">💾 儲存所有記錄</button>
        </div>
    </div>

    <!-- 位置交換：現在排行榜在上方 -->
    <div class="card">
        <h3>🏆 股票表現排行榜 <span class="small-text">含已實現損益 (賣出賺蝕)</span></h3>
        <div class="small-text" style="margin-bottom: 10px;">
            * <b>股息率 (Yield)</b> = 總股息 / 歷史總投入成本<br>
            * <b>總回報 (Total)</b> = (市值 + 股息 + 賣出取回 - 總投入)
        </div>
        <div style="overflow-x: auto;">
            <table id="roi-table">
                <thead>
                    <tr>
                        <th onclick="sortRoi('code')">股票 <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('totalInvested')">總投入成本 <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('totalDiv')">總股息 <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('realizedPnl')">已平倉損益 <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('yield')">股息率 <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('totalProfit')">總回報 ($) <span class="sort-icon"></span></th>
                        <th onclick="sortRoi('totalRetPct')">總回報率 (%) <span class="sort-icon"></span></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <!-- 位置交換：現在持倉在下方 -->
    <div class="card">
        <h3>💼 持倉現況 <button class="btn btn-success" style="font-size:0.8rem; padding:4px 8px;" onclick="fetchYahooPrices()">🔄 更新現價</button></h3>
        <div class="small-text">點擊股票行可查看詳細資料</div>
        <div style="overflow-x: auto;">
            <table id="portfolio-table">
                <thead>
                    <tr>
                        <th width="20%" onclick="sortPortfolio('code')">股票 <span class="sort-icon"></span></th>
                        <th width="15%" onclick="sortPortfolio('qty')">持倉股數 <span class="sort-icon"></span></th>
                        <th width="15%" onclick="sortPortfolio('avgPrice')">平均買入價 <span class="sort-icon"></span></th>
                        <th width="15%">現價 (可輸入)</th>
                        <th width="20%" onclick="sortPortfolio('marketVal')">市值 <span class="sort-icon"></span></th>
                        <th width="15%" onclick="sortPortfolio('paperPnl')">帳面盈虧 <span class="sort-icon"></span></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    <div class="card" style="border: 2px dashed var(--primary); background: #f9fbff;">
        <h3>📤 Excel 數據批量匯入 (專業版邏輯)</h3>
        <div class="import-grid">
            <div>
                <label><b>1. 月供記錄表 (v5.0 邏輯)</b></label>
                <textarea id="excel-buy-area" rows="5" placeholder="貼上月供表範圍..."></textarea>
                <button class="btn btn-primary" style="width:100%; margin-top:10px;" onclick="importBuyData()">匯入買入及股息</button>
            </div>
            <div>
                <label><b>2. 賣出股票表 (v5.2 邏輯 A1:X4)</b></label>
                <textarea id="excel-sell-area" rows="5" placeholder="貼上賣出表 A1:X4 範圍..."></textarea>
                <button class="btn btn-danger" style="width:100%; margin-top:10px;" onclick="importSellData()">匯入賣出記錄</button>
            </div>
        </div>
    </div>
    ...
    <div class="card">
            ...
            <table id="history-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 10%">股數</th>
                        <th style="width: 12%">手續費</th>
                        <th style="width: 15%">總變動</th>
        ...
        <div style="margin-top: 10px; text-align: right;">
            <button class="btn btn-sm btn-danger" onclick="clearAllData()">⚠️ 清空所有資料</button>
        </div>
    </div>
...
<script>
    // --- 內建股票中文名稱映射 (根據用戶提供) ---
    const stockMap = {
        "0005.HK": "滙豐控股", "0012.HK": "恒基地產", "0027.HK": "銀河娛樂",
        "0388.HK": "香港交易所", "0823.HK": "領展房產", "0883.HK": "中海油",
        "2638.HK": "港燈-SS", "2800.HK": "盈富基金", "0001.HK": "長和",
        "0003.HK": "煤氣", "0011.HK": "恒生銀行", "0016.HK": "新鴻基",
        "0066.HK": "港鐵", "0700.HK": "騰訊", "09988.HK": "阿里",
        "03690.HK": "美團", "01299.HK": "友邦", "00939.HK": "建行",
        "01398.HK": "工行", "03988.HK": "中行"
    };
        ...
        transactions.forEach(t => {
            if (!p[t.code]) {
                p[t.code] = { 
                    qty: 0, 
                    currentCost: 0, 
                    totalInvested: 0, // 含買入手續費
                    totalDiv: 0, 
                    soldCash: 0, // 淨收回現金 (已扣除賣出手續費)
                    soldCost: 0,
                    sellFees: 0, // 累積賣出手續費
                    grossSold: 0 // 賣出總金額 (未扣手續費)
                };
            }
            let stock = p[t.code];
    ...
    // 1. 渲染 KPI (更新：總投入包含賣出手續費)
    function renderKPI(p) {
        let totalCost = 0;   // 目前持倉成本
        let totalMarket = 0; // 目前市值
        let totalDiv = 0;    // 總股息
        let totalSoldCash = 0; // 淨賣出回收現金
        let totalInvestedAllTime = 0; // 歷史買入總投入
        let totalSellFees = 0; // 總賣出手續費
        ...
        // 請求：總投入成本 (本金) 應該要包括賣出股票的手續費
        // 定義：修正後總投入 = 買入總成本 + 賣出手續費
        const adjustedTotalInvested = totalInvestedAllTime + totalSellFees;

        // 淨回報 = (目前市值 + 總股息 + 淨賣出回收) - 買入總投入
        // 數學上等同於： (市值 + 股息 + 賣出Gross) - (買入投入 + 賣出手續費)
        ...
        pnlEl.className = totalReturn >= 0 ? 'kpi-val text-up' : 'kpi-val text-down'; // 綠紅變色
                ...
                <tr id="${rowId}" class="detail-row">
                    <td colspan="6">
                        <div class="detail-content">
                            <div class="detail-item"><strong>持倉成本</strong> $${fmtMoney(r.data.currentCost)}</div>
                            <div class="detail-item"><strong>歷史總股息</strong> $${fmtMoney(r.data.totalDiv)}</div>
                            <div class="detail-item"><strong>已變現金額</strong> $${fmtMoney(r.data.soldCash)}</div>
                            <div class="detail-item"><strong>歷史總投入</strong> $${fmtMoney(r.data.totalInvested)}</div>
                            <div class="detail-item"><strong>已平倉損益</strong> <span class="${(r.data.soldCash - r.data.soldCost)>=0?'text-up':'text-down'}">$${fmtMoney(r.data.soldCash - r.data.soldCost)}</span></div>
                            <div class="detail-item"><strong>賣出手續費</strong> $${fmtMoney(r.data.sellFees)}</div>
                        </div>
                    </td>
                </tr>
            `;
    ...
    function renderROITable(p) {
        ...
        for (let c in p) {
            ...
            // 修正：分母用 Adjusted Invested (買入成本+賣出費)
            ...
            rows.push({
                code: c,
                name: getStockName(c),
                totalInvested: adjustedInv, // 顯示含手續費的總投入
                totalDiv: s.totalDiv,
    ...
    function renderCharts(p) {
        ...
        let accRealizedPnl = 0; // 累積平倉損益
        let accDiv = 0;         // 累積股息
        let accHoldingCost = 0; // 持倉成本 (買入 - 賣出成本)
        ...
        trendChartInst1 = new Chart(document.getElementById('trendChart1'), {
            ...
            data: {
                labels: dates,
                datasets: [
                    { label: '累積平倉回報 (已實現損益)', data: dataRealized, borderColor: '#722ed1', backgroundColor: 'rgba(114,46,209,0.1)', fill: true },
                    { label: '累積股息收入', data: dataDiv, borderColor: '#52c41a', backgroundColor: 'rgba(82,196,26,0.1)', fill: true }
                ]
            },
            options: { maintainAspectRatio: false, plugins: { title: { display: true, text: '回報趨勢 (平倉 vs 股息)' } } }
        });
        ...
        trendChartInst2 = new Chart(document.getElementById('trendChart2'), {
            ...
            data: {
                labels: dates,
                datasets: [
                    { label: '總持倉成本趨勢', data: dataHoldingCost, borderColor: '#1890ff', backgroundColor: 'rgba(24,144,255,0.1)', fill: true }
                ]
            },
            options: { maintainAspectRatio: false, plugins: { title: { display: true, text: '持倉規模趨勢 (成本法)' } } }
        });

        // Pie Chart: 佔比 (含百分比 Label)
        ...
        barChartInst = new Chart(document.getElementById('barChart'), {
            type: 'bar',
            data: { labels: Object.keys(yearly), datasets: [{ label: '年度股息收入', data: Object.values(yearly), backgroundColor: '#13c2c2' }] },
    ...
    function renderHistory() {
        ...
        list.forEach(t => {
            ...
            if (m !== currentMonth) {
                currentMonth = m;
                tbody.innerHTML += `<tr class="history-month-row"><td colspan="8">${m.replace('-', '年')}月</td></tr>`;
            }

            let act = '', valClass = '';
            if(t.type==='buy') { act='買入'; valClass='text-down'; }
            else if(t.type==='sell') { act='賣出'; valClass='text-up'; }
            else if(t.type.includes('cash')) { act='收息(錢)'; valClass='text-up'; }
            else if(t.type.includes('scrip')) { act='收息(貨)'; valClass='text-up'; }
            ...
            else if (t.type === 'dividend_scrip') totalStr = `+${t.qty}股`;
        ...
        tr.innerHTML = `
            <td>
                <select class="row-type" onchange="handleTypeChange(this)">
                    <option value="buy" ${type==='buy'?'selected':''}>買入</option>
                    <option value="dividend_cash">收息 (派錢)</option>
                    <option value="dividend_scrip">收息 (派貨)</option>
                    <option value="sell">賣出</option>
                </select>
            </td>
            <td><input type="text" class="row-code" value="${code}" placeholder="0005.HK" oninput="this.value=this.value.toUpperCase()"></td>
            <td><input type="number" class="row-price" step="0.01" placeholder="單價"></td>
            <td><input type="number" class="row-qty" value="${qty}" placeholder="股數"></td>
    ...
    function handleTypeChange(select) {
        ...
        if(type === 'dividend_cash') {
            priceInp.placeholder = "總金額 ($)"; qtyInp.disabled = true; qtyInp.placeholder = "-";
        } else if(type === 'dividend_scrip') {
            priceInp.disabled = true; priceInp.placeholder = "0"; qtyInp.placeholder = "收到股數";
        } else {
            priceInp.placeholder = "單價"; qtyInp.placeholder = "股數";
        }
    }
    ...
    function saveBatchTransaction() {
        ...
        newTrans.forEach(t => {
            ...
            if(t.type === 'sell') { 
                // 賣出手續費邏輯：如果用戶在批量輸入裡賣出，這裡無法得知該次賣出的準確手續費分配
                // 假設 globalFee 對賣出也生效？通常用戶不會同時混和輸入買賣。
                // 這裡簡單處理：如果是賣出，將 globalFee 從回收金額中扣除 (total變小)
                // 這裡 t.total 目前是 Gross. 
                // 我們統一邏輯：t.total 在 DB 存的是「淨現金流」
                // 所以 Buy: total = Cost + Fee (負向現金流的絕對值)
                // Sell: total = Revenue - Fee (正向現金流)
                // 這裡如果是 Sell, t.total = val1*val2 (Gross). 
                // 為了保持一致，如果用戶輸入了手續費，我們扣除它。
                // *注意*：如果上方輸入了手續費，會從賣出金額扣除。
                if(t.price > 0 && globalFee > 0) {
                     t.fee = globalFee; // 假設單筆賣出全吃
                     t.total -= t.fee;
                }
            }
        });
    ...
    async function fetchYahooPrices() {
        ...
        if (codes.length === 0) return alert("目前沒有持倉需要更新");
        let updated = 0;
    ...
    function importBuyData() {
        ...
        for(let col=0; col < header.length; col++) {
            ...
            if(code && (code.includes('.') || !isNaN(code)) && code !== '日期' && code !== '手續費') {
                 if(!code.includes('.')) code += ".HK";
        ...
        for(let i=1; i<lines.length; i++) {
            const r = lines[i];
            if(!r[0] || r[0].includes('年')) continue;
        ...
        saveAndRefresh();
        alert(`匯入成功！已加入 ${newItems.length} 筆記錄。`);
    ...
    function importSellData() {
        ...
        saveAndRefresh();
        alert(`成功匯入 ${newSells.length} 筆賣出記錄`);
